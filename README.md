...
Almost everything underneath it belongs to other people.
...
platform — the APF bridge, ROM/data loader, video mixer and scaler, audio path
and filters, Interact option decoding, memory primitives, hiscore support — and
41 of its 64 files carry his copyright. The Quartus project files
(`projects/xeno_pocket.q*`) were generated by his Gateman CLI,
`target/pocket/core_top.sv` starts from his template, and both CI and
`build-local.sh` compile inside `raetro/quartus:pocket`, his containerised
Quartus toolchain. The Docker image is the most visible piece and the smallest
one: without the platform layer this project would have opened with months of
Pocket plumbing instead of MCR-68000 hardware.

**Jorge Cwik ([@ijor](https://github.com/ijor)) — [fx68k](https://github.com/ijor/fx68k).**
The cycle-accurate 68000 in `modules/cpu-fx68k`, instantiated twice: the main
CPU and the Sounds Good sound CPU.

**Alexey Melnikov (Sorgelig).** The MiSTer audio filter chain, DC blocker and
audio mixer, the scanline and shadowmask generators, and — with **Mike
Field** — the SDRAM controller this core's `rtl/sdram16.sv` is based on.

Also in the build:

- **Adam Gastineau** — the Pocket data loader and unloader.
- **Jim Gregory** and **Alan Steremberg** — MAME hiscore.dat support.
- **Jacob Boline** — USB HID and PS/2 keyboard translation.
- **Till Harbaum** — the original scanline generator.
- **Analogue** — the openFPGA APF reference `core_top`.

**The MAME team.** MAME's `mcr68.cpp` and `midway_sound.cpp` documented the
board, and MAME is the reference the video regression in `tools/regress_video.sh`
